...
# encoding: utf-8
"""
Monitors a queue for feeds with link entities to take
...
class FeedUrlWorker(object):
    def __init__(self, heart):
        super(FeedUrlWorker, self).__init__()
        self.heart = heart
    ...
    def run(self):
        ...
        while True:
            ...
            if job:
                try:
                    feed = anyjson.deserialize(job.body)
                    self.process_entities(feed)
                    job.delete()
                except Exception as e:
                    log.error("Exception caught, burying screenshot job for feed {feed}: {e_type} {e}",
                              feed=feed.get('id'), e=e, e_type=type(e))
                    job.bury()

    def process_entities(self, feed):
        """
        entities = []
        entities_key = 'extended_entities' if 'extended_entities' in feed else 'entities'
        entities += dict_mget(feed, entities_key, 'urls') or []
        entities += dict_mget(feed, entities_key, 'media') or []
        ...
        for entity_index, url_entity in enumerate(entities):
            ...
            if len(urls) == 0:
                log.info("No URLs for entity {entity} on feed {feed}. Skipping.",
                         **log_context)
                return

            log.info("URLs for entity {entity} on feed {feed}: {urls}",
                      **log_context)
        """
        ...
        for entity_index, url in enumerate(urls):
            ...
            if response.status_code != httplib.OK:
                log.warn("Unable to retrieve head for feed {feed} entity URL {url}",
                         url=url,
                         feed=feed.get('id'))

            if response.headers.get('content-type', '').startswith('image/'):
                self.mirror_entity_image(feed, entity_index, url)
            else:
                self.screenshot_entity_url(feed, entity_index, url)
            time.sleep(1)

    def record_feed_image(self, feed, url):
        with database_cursor(**self.db_connect_params) as cursor:
            cursor.execute("""INSERT INTO `feed_images` (`feed_id`, `url`, `created_at`, `updated_at`) VALUES(%s, %s, NOW(), NOW())""", (feed['id'], url))
            log.info("Inserted image into database for feed {feed}: {url}",
                      feed=feed.get('id'), url=url)


    def screenshot_entity_url(self, feed, entity_index, url):
        filename = "{feed}-{index}.png".format(feed=feed.get('id'),
                                                index=entity_index)
        ...
        with NamedTemporaryFile(mode='wb', prefix='twoops', suffix='.png', delete=True) as fil:
            ...
            if new_url:
                self.record_feed_image(feed, new_url)

    def mirror_entity_image(self, feed, entity_index, url):
        response = requests.get(url, allow_redirects=True, timeout=15)
        if response.status_code != httplib.OK:
        ...
        filename = "{feed}-{index}{extension}".format(feed=feed.get('id'),
                                                       index=entity_index,
                                                       extension=extension)
